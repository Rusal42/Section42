...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('cancel')
                .setDescription('Cancel an active giveaway (no winners)')
                .addStringOption(option =>
                    option.setName('message_id')
                        .setDescription('The giveaway message ID')
                        .setRequired(true)))
...
const GIVEAWAY_ROLE_ID = '1512975288278061076';

async function handleStart(interaction) {
    if (!interaction.member.permissions.has(PermissionFlagsBits.ManageMessages) && !interaction.member.roles.cache.has(GIVEAWAY_ROLE_ID)) {
    ...
    await interaction.followUp({
        content: `Giveaway started! Message ID: \`${giveawayId}\` (use this with /giveaway end or /giveaway cancel)`,
        flags: 64
    });
}
